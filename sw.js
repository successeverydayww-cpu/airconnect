...
self.addEventListener('install', e => {
  e.waitUntil(caches.open('airconnect-v11').then(c => c.addAll(SHELL)).then(() => self.skipWaiting()));
});
self.addEventListener('activate', e => {
  e.waitUntil(caches.keys().then(ks => Promise.all(ks.filter(k => k !== 'airconnect-v11').map(k => caches.delete(k)))).then(() => self.clients.claim()));
});
  ...
  if (e.request.mode === 'navigate') {
    e.respondWith(
      fetch(e.request).then(r => {
        const cp = r.clone();
        caches.open('airconnect-v11').then(c => c.put(e.request, cp)).catch(() => {});
        return r;
